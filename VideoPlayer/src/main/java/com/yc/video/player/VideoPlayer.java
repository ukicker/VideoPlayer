...
public class VideoPlayer<P extends AbstractVideoPlayer> extends FrameLayout
    ...
    FloatVideoView mFloatVideoView;
    ...
    public void startTinyScreen() {
        ...
        Activity activity = VideoPlayerHelper.instance().getActivity(mContext, mVideoController);
        if (activity != null) {
            activity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
        }

        int y = PlayerUtils.getScreenHeight(getContext()) + PlayerUtils.getScreenHeight(getContext()) / 3;
        if (mFloatVideoView == null) {
            mFloatVideoView = new FloatVideoView(getContext(),  y);
        }
        mFloatVideoView.setClipToOutline(PlayerUtils.dp2px(getContext(), 10));
        this.removeView(mPlayerContainer);
        ViewGroup viewGroup = (ViewGroup) mPlayerContainer.getParent();
        if (viewGroup != null) {
            viewGroup.removeView(mPlayerContainer);
        }
        mFloatVideoView.removeAllViews();
        mFloatVideoView.removeFromWindow();
        mFloatVideoView.addView(mPlayerContainer);
        VideoViewManager.instance().instance().addPipView(mFloatVideoView);
        mIsTinyScreen = true;
        mIsFullScreen = false;
    ...
    public void stopTinyScreen() {
        ...
        mIsTinyScreen = false;
        setPlayerState(ConstantKeys.PlayMode.MODE_NORMAL);
        VideoViewManager.instance().instance().releaseByTag("PIP");
    }
